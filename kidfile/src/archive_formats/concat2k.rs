...
// finding 2048 byte boundaries following 256 null bytes and at least one non-null byte in the next 64 bytes
...
pub const ENTRY_CONCAT2K: Decoder<Archive> = Decoder {
	...
	detect: |file| Certainty::possible_if(file.len() > ALIGNMENT * 2 && !file.starts_with(b"\x7FELF") && !file.starts_with(b"\0\0\x01\xBA") && {
		...
		loop {
			let mut check_buf = [0u8; 256 + 64];
			if file.read_chunk_exact(&mut check_buf, boundary - 256).is_err() {
				break false;
			}
			if check_buf[0..256].iter().all(|x| *x == 0) && check_buf[256..256 + 64].iter().any(|x| *x != 0) {
				break true;
			}
			boundary += ALIGNMENT;
		}
...
fn decode(file: &mut FileData) -> Result<Archive, String> {
